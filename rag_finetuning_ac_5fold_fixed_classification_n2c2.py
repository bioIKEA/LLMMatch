...
bnb_config = BitsAndBytesConfig(
    ...
    bnb_4bit_compute_dtype=torch.float16  # Switch from bfloat16 if necessary
)
...
base_model = AutoModelForCausalLM.from_pretrained(
    model_id,
    device_map="auto",  # Force all parameters onto one device
...
# Override the forward method to ensure all inputs are on the same device
def new_forward(input_ids, attention_mask=None, labels=None, **kwargs):
    ...
    # Extract the last hidden state (final layer representation)
    ...
    logits = model.classifier(cls_token_output)  # Shape: (batch_size, num_classes)
...
data_collator = DataCollatorWithPadding(tokenizer) # Keeps labels as integers

df_fine_tune_train["label"] = df_fine_tune_train["response"].map({"Yes": 1, "No": 0})
df_fine_tune_test["label"] = df_fine_tune_test["response"].map({"Yes": 1, "No": 0})
...
training_args = TrainingArguments(
    output_dir=f"./checkpoints_classification",
    ...
    fp16=True,
    save_total_limit=2,
    logging_dir=f"./logs_fold_classification",
    logging_steps=10,
...
# Modify save function to use torch.save
trainer.save_model = partial(trainer.save_model, save_function=torch.save)
...
output_dir = f"./fine_tuned_peft_model_classification"
...
save_dir = f"./fine_tuned_complete_model_classification"
